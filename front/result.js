...
function characterResult() {
  ...
  //   // 출현 빈도가 높은 순으로 정렬
  //   var sortedCharacters = Object.entries(characterCounts).sort(
  //     (a, b) => b[1] - a[1]
  //   );
  //   var highestFrequency = sortedCharacters[0][1];

  //   // 가장 높은 빈도를 가진 캐릭터가 여러 개인지 확인
  //   var mostFrequentCharacters = sortedCharacters.filter(
  //     (character) => character[1] === highestFrequency
  //   );
  ...
  //       resultText =
  //         mostFrequentCharacters.length > 0
  //           ? nameMappingCN[characterKey] + "专家"
  //           : "无结果"; // 중국어로 "전문가", "결과 없음"
  //       imagePath =
  //         mostFrequentCharacters.length > 0 ? imageMapping[characterKey] : ""; // 해당 캐릭터의 이미지 경로
  //     }
  ...
  //       resultText =
  //         mostFrequentCharacters.length > 0
  //           ? nameMappingKO[characterKey] + " 전문가"
  //           : "결과 없음";
  //       imagePath =
  //         mostFrequentCharacters.length > 0 ? imageMapping[characterKey] : ""; // 해당 캐릭터의 이미지 경로
  //     }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (!isInTopFive && userRanking && userRanking.rankingCount) {
    ...
    // 캐릭터 타입을 현재 언어 설정에 맞게 변환하여 표시
    var typesTextRank6 = userRanking.characters
      .map(function (character) {
        var characterName = character; // 기본값 설정
        if (preferredLanguage === "cn") {
...
$(document).ready(function () {
  ...
  $(".push").click(function () {
    ...
    $.ajax({
      url: `http://localhost:8080/comment/${userId}`,
      type: "POST",
  ...
  $(document).on("click", ".user_list .retweet-btn", function (event) {
    ...
    console.log("comment: ", comment); // 가져온 텍스트를 콘솔에 출력

    // AJAX 요청
    $.ajax({
      url: `http://localhost:8080/comment/translate`, // 요청을 보낼 서버의 URL 주소
